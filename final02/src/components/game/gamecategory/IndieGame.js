...
const CATEGORY_CONFIG = {
  name: 'IndieGame',
  filterKey: 'IndieGame' // API 필터링에 사용될 키워드
};
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 인디 게임 필터링
          const IndieGame = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...IndieGame].sort((a, b) => {
            switch(sorting) {
              case 'popular':
