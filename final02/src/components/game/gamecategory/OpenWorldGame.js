/**
 * OpenWorldGame.js
 * OpenWorldGame 게임 카테고리 페이지를 위한 메인 컴포넌트
...
const CATEGORY_CONFIG = {
  name: 'Open World',
  description: '광활한 오픈월드에서 자유롭게 모험을 즐겨보세요',
  tags: ['오픈월드', '탐험', '자유도', '샌드박스'],
  filterKey: 'openworld'
};
...
const GameCard = ({ game }) => {
  ...
  /**
   * OpenWorldGame 메인 컴포넌트
   * OpenWorldGame 게임 카테고리 페이지의 전체 구조를 정의
   */
  const OpenWorldGame = () => {
    // 상태 관리
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // OpenWorldGame 게임 필터링
          const OpenWorldGame = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...OpenWorldGame].sort((a, b) => {
            switch(sorting) {
              case 'popular':
