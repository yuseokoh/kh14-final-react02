...
const CATEGORY_CONFIG = {
  name: 'Roguelike',
  description: '도전적인 로그라이크 게임으로 새로운 모험을 경험하세요',
  tags: ['로그라이크', '던전', '랜덤', '퍼머데스'],
  filterKey: 'roguelike'
};
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 로그라이크 게임 필터링
          const Roguelike = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...Roguelike].sort((a, b) => {
            switch(sorting) {
              case 'popular':
