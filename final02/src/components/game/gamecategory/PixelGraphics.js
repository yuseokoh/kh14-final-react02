...
const CATEGORY_CONFIG = {
  name: 'Pixel Graphics',
  description: '클래식한 픽셀 아트 스타일의 게임을 즐겨보세요',
  tags: ['픽셀아트', '레트로', '2D', '클래식'],
  filterKey: 'pixel'
};
...
const GameCard = ({ game }) => {
  ...
  /**
   * PixelGraphics 메인 컴포넌트
   * 픽셀 게임 카테고리 페이지의 전체 구조를 정의
   */
  const PixelGraphics = () => {
    // 상태 관리
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 픽셀 게임 필터링
          const PixelGraphics = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...PixelGraphics].sort((a, b) => {
            switch(sorting) {
              case 'popular':
