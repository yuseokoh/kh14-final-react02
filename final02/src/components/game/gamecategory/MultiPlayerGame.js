...
const CATEGORY_CONFIG = {
  ...
  description: '친구들과 함께 즐기는 멀티플레이어 게임을 만나보세요', // 추가 필요
  tags: ['Co-op', 'PvP', '멀티플레이어', '온라인', 'MMO'], // 추가 필요
  filterKey: 'MultiPlayerGame'
};
...
const GameCard = ({ game }) => {
  ...
  /**
   * MultiPlayerGame 메인 컴포넌트
   * 멀티플레이 게임 카테고리 페이지의 전체 구조를 정의
   */
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 정렬 적용
          const sortedGames = [...MultiPlayerGame].sort((a, b) => {
            switch(sorting) {
              case 'popular':
