...
const CATEGORY_CONFIG = {
  name: 'Survival',
  description: '극한의 환경에서 살아남는 서바이벌 게임을 체험하세요',
  tags: ['생존', '크래프팅', '자원관리', 'PvE'],
  filterKey: 'survival'
};
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 서바이벌 게임 필터링
          const Survival = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...Survival].sort((a, b) => {
            switch(sorting) {
              case 'popular':
