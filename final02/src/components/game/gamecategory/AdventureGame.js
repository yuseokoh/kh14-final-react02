...
const CATEGORY_CONFIG = {
  name: 'Adventure',
  description: '미스터리와 탐험이 가득한 어드벤처 게임을 즐겨보세요',
  tags: ['탐험', '퍼즐', '스토리', '미스터리'],
  filterKey: 'adventure'
};
/**
...
const GameCard = ({ game }) => {
  ...
  /**
   * AdventureGame 메인 컴포넌트
   * 어드벤쳐 게임 카테고리 페이지의 전체 구조를 정의
   */
  const AdventureGame = () => {
    // 상태 관리
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 어드벤쳐 게임 필터링
          const AdventureGame = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...AdventureGame].sort((a, b) => {
            switch(sorting) {
              case 'popular':
