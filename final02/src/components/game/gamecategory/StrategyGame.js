...
const CATEGORY_CONFIG = {
  name: 'Strategy',
  description: '전략적 사고가 필요한 전략 게임에 도전해보세요',
  tags: ['전략', '전술', 'RTS', '턴제'],
  filterKey: 'strategy'
};
...
const GameCard = ({ game }) => {
  ...
  /**
   * StrategyGame 메인 컴포넌트
   * 전략 게임 카테고리 페이지의 전체 구조를 정의
   */
  const StrategyGame = () => {
    // 상태 관리
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 전략StrategyGame 게임 필터링
          const StrategyGame = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...StrategyGame].sort((a, b) => {
            switch(sorting) {
              case 'popular':
