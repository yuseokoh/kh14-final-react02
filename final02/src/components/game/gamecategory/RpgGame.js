...
const CATEGORY_CONFIG = {
  name: 'RPG',
  description: '깊이 있는 스토리와 캐릭터 육성의 RPG를 체험해보세요',
  tags: ['롤플레잉', '캐릭터육성', '스토리', '퀘스트'],
  filterKey: 'rpg'
};
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // RPG 게임 필터링
          const RpgGame = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...RpgGame].sort((a, b) => {
            switch(sorting) {
              case 'popular':
