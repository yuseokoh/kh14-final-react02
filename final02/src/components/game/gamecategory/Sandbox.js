...
const CATEGORY_CONFIG = {
  name: 'Sandbox',
  description: '자유로운 창작과 실험이 가능한 샌드박스 게임을 즐겨보세요',
  tags: ['샌드박스', '제작', '자유도', '창의성'],
  filterKey: 'sandbox'
};
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 샌드박스 게임 필터링
          const Sandbox = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...Sandbox].sort((a, b) => {
            switch(sorting) {
              case 'popular':
