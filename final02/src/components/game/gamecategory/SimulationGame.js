...
const CATEGORY_CONFIG = {
  name: 'Simulation',
  description: '현실적인 시뮬레이션으로 다양한 경험을 해보세요',
  tags: ['시뮬레이션', '경영', '생활', '직업'],
...
const GameCard = ({ game }) => {
  ...
  /**
   * SimulationGame 메인 컴포넌트
   * 시뮬레이션 게임 카테고리 페이지의 전체 구조를 정의
   */
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 시뮬레이션 게임 필터링
          const SimulationGame = response.data.filter(game =>
            game.gameCategory.toLowerCase().includes(CATEGORY_CONFIG.filterKey)
          );
  
          // 정렬 적용
          const sortedGames = [...SimulationGame].sort((a, b) => {
            switch(sorting) {
              case 'popular':
