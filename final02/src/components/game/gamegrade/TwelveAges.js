...
const CATEGORY_CONFIG = {
  name: '12세 이용가',
  description: '12세 이상이 이용할 수 있는 게임을 만나보세요',
  tags: ['어드벤처', '액션', '캐주얼', '시뮬레이션', '스포츠', '전략'],
  filterKey: '12' 
};
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        ...
        // 12세 이용가 게임만 필터링
        const twelveAgesGames = response.data.filter(game =>
          game.gameGrade === "12세이용가"
        );

        // 정렬 적용
        const sortedGames = [...twelveAgesGames].sort((a, b) => {
          switch(sorting) {
            case 'popular':
