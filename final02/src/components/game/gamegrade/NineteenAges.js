/**
 * NineteenAges.js
 * 19세 이용가 게임 카테고리 페이지를 위한 메인 컴포넌트
 */
...
const CATEGORY_CONFIG = {
  name: '19세 이용가',
  description: '성인이 이용할 수 있는 게임을 만나보세요',
  tags: ['액션', '공포', 'RPG', '전략', '서바이벌', 'FPS'],
  filterKey: '19' 
};
      ...
      const loadGameImage = async () => {
        ...
        try {
          ...
          if (response.data && response.data.length > 0) {
            const url = `/game/download/${response.data[0].attachmentNo}`;
            setImageUrl(url);
          }
        } catch (error) {
    ...
    useEffect(() => {
      const fetchGames = async () => {
        try {
          ...
          // 19세 이용가 게임만 필터링
          const nineteenAgesGames = response.data.filter(game =>
            game.gameGrade === "19세이용가"
          );
  
          // 정렬 적용
          const sortedGames = [...nineteenAgesGames].sort((a, b) => {
            switch(sorting) {
              case 'popular':
