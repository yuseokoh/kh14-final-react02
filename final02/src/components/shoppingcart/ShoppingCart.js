    ...
    try {
      ...
      for (const cart of cartListData) {
        ...
        if (response.data && response.data.length > 0) {
          ...
          newImageUrls[cart.cartId] = 'https://via.placeholder.com/200'; // 이미지가 없을 경우 placeholder 사용
        }
      }
      ...
      <div className={styles.cartItemsContainer}>
        {cartList.map(cart => (
          <div key={cart.cartId} className={styles.cartItem}>
            {/* 게임 썸네일 */}
            <img
              src={imageUrls[cart.cartId] || 'https://via.placeholder.com/200'} // 이미지가 없으면 placeholder 사용
              alt={cart.gameTitle}
